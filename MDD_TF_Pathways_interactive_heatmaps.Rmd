...
Mean summarize replicate RPPA pathway scores  
Find the correlations betwwen the top TFs and the RPPA pathways across all 24 and 48 hour conditions and display as a heatmap  
...
pathway <- "Apoptosis_Pathway_Score"
df <- top_TF_pathways %>%
  gather(key = "condition", value = "value", -feature) %>%
  filter(str_detect(feature,pathway)) %>%
  spread(key = feature, value = value) %>%
...
p <- ggplot(df, aes_string(pathway, "TF", colour = "condition")) +
  geom_point() +
  facet_wrap(~feature)
print(p)
