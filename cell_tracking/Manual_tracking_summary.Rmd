...
df_input <- dir(paste0(plateID,"/Data"), pattern = paste0(plateID,".*_lineages.csv$"), full.names = TRUE) %>%
  map(read_csv,
      col_types = cols(
        .default = col_double(),
        D2Rpixel = col_number(),
...
df_orphans <-  dir(paste0(plateID,"Data"), pattern = paste0(plateID,".*_orphans.csv$"), full.names = TRUE) %>%
  map(read_csv,
      col_types = cols(
        tmin = col_character(),
        TID = col_character(),
