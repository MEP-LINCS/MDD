...
knitr::opts_chunk$set(echo = FALSE, cache=TRUE, cache.lazy = FALSE, message=FALSE, warning = FALSE, fig.height=6, fig.width=12)
...
df <- pk_selected_rr %>%
  ...
  mutate(direction = set_membership(value, zscore_thresh = zscore_thresh),
         experimentalCondition = factor(experimentalCondition, levels = c("PBS_24", "IFNG_24", "TGFB_24", "OSM_24", "HGF_24", "EGF_24", "BMP2_24"), ordered = TRUE)) %>%
  filter(!direction == 0)
