...
plot_iheatmap_ver <- function(df, assay_name, cluster_rows = FALSE, cluster_columns = TRUE, ...) {
  ...
  hm <- main_heatmap(data = df_as_matrix,
    ...
    add_row_labels(font = list(size = 2),
                   side = "right") %>%
    add_col_labels() %>%
...
Clustering  
The hierarcical clustering uses the complete linkage method to find similar clusters. The number of clusters as shown in the side annotation bar is arbitrary chosen as 6. The scatter plot above the heatmap shows the mean value for each cluster within each condition.  
