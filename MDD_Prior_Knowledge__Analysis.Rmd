...
md <- read_csv("MDD/metadata/MDD_sample_annotations.csv")
...
TFs_values <- get_TF_values(dir_path = "MDD/RNAseq/Data/ChEA3_results_MD_OHSU", pattern = "ctrl_vs_.*xlsx", sheet =1)
TFs_details <- map(2:7, get_TF_values, dir_path = "MDD/RNAseq/Data/ChEA3_results_MD_OHSU", pattern = "ctrl_vs_.*xlsx") %>%
  bind_rows()
...
hallmark_pathways <- read_csv("MDD/RNAseq/Data/MDD_RNAseq_HallmarkNES.csv") %>%
...
RPPA_values <- read_csv("MDD/RPPA/Data/MDD_RPPA_Level3.csv") %>%
...
cycIF_values <- read_csv("MDD/cycIF/Data/MDD_cycIF_Level3.csv") %>%
...
GCP_values <- read_csv("MDD/GCP/Data/MDD_GCP_Level3.csv") %>%
...
pk_top_rr <- pk_top %>%
  group_by(feature) %>%
  mutate(value = rrscaleVector(value),
...
df <- pk_top_rr %>%
  ...
  right_join(pk_top) %>%
  dplyr::select(condition, feature, Type, value, value_rr) %>%
