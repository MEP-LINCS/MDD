...
plot_iheatmap_ver <- function(df, assay_name, cluster_rows = FALSE, cluster_columns = TRUE, ...) {
  ...
  hm <- main_heatmap(data = df_as_matrix,
    ...
    add_row_labels(font = list(size = 6),
                   side = "right") %>%
    add_col_labels() %>%
    add_row_annotation(ann_df,
                       side = "left",
                       size = 0.05) %>%
    add_row_clustering(name = "Cluster",k = 6) %>%
...
ATACseq_T0_norm_med <- pk_preprocess_level3(ATACseq_values, type = "ATACseq") %>%
  left_join(ATACseq_metadata, by = c("feature" = "peak")) %>%
  drop_na() %>%
  select(-feature) %>%
  rename(feature = hgnc_symbol) %>%
  mutate(feature = paste0(feature, "_ATAC"))

#filter ATACseq features on variance
ATACseq_variance_probs_thresh <- .99
ATACseq_display <- ATACseq_T0_norm_med %>%
  group_by(feature) %>%
  mutate(feature_variance = var(value, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(feature_variance > quantile(feature_variance,
                                     probs = ATACseq_variance_probs_thresh,
                                     na.rm = TRUE)) %>%
  dplyr::select(-feature_variance) %>%
  group_by(feature, condition, Type) %>%
...
ATACseq_Motif_T0_norm_med <- pk_preprocess_level3(ATACseq_Motif_values, type = "ATACseq_motif") %>%
  drop_na()
...
IF_T0_norm_med <- pk_preprocess_level3(IF_values, type = "IF") %>%
  drop_na()

pk_all <- bind_rows(TFs, RPPA_pathways, hallmark_pathways, RPPA_T0_norm_med, cycIF_T0_norm_med, GCP_T0_norm_med, ATACseq_display, ATACseq_Motif_T0_norm_med, IF_T0_norm_med) %>%
...
pk_top <- bind_rows(top_TFs, RPPA_pathways, hallmark_pathways, GCP_display, RPPA_display, ATACseq_display, cycIF_T0_norm_med,  ATACseq_Motif_T0_norm_med, IF_T0_norm_med) %>%
  filter(str_detect(condition, "24|48"),
...
This analysis combines MDD RNAseq, RPPA and ATACseq data with prior knowledge of transcription factors, pathways and motifs. It then adds in selected cycIF, RPPA, GCP, ATACseq and IF values. 
...
Median summarize the values across the replicates  
Select the GCP histones with a variance above the `r GCP_variance_probs_thresh*100`th percentile. This yields `r length(unique(GCP_display$feature))` high variance histones in the heatmap.  

ATACseq Values   
Filter the ATACseq peaks to those annotated to be within Promoter, 3' UTR, 5' UTR, or 1st Exon sections    
Replace the peak name with the HGNC gene name  
If there is more than one peak within the same gene, choose the maximum value of all peaks  
T0 normalize the ATACseq values within each replicate set  
Median summarize the values across the replicates  
Select the genes with a variance above the `r ATACseq_variance_probs_thresh*100`th percentile   
This yields `r length(unique(ATACseq_display$feature))` high variance genes in the heatmap   

ATACseq Motifs  
T0 normalize the ATACseq Motif values within each replicate set  
Median summarize the values across the replicates  
There are `r length(unique(ATACseq_Motif_T0_norm_med$feature))` motifs in the heatmap.  

IF Values  
Filter out the CellMask intensities and use the remaining `r length(unique(IF_T0_norm_med$feature))` IF features  
T0 normalize the values within each replicate set  
Median summarize the values across the replicates  

Clustering  
The hierarchical clustering uses the complete linkage method to find similar clusters. The number of clusters as shown in the side annotation bar is arbitrarily chosen as 6. The scatter plot above the heatmap shows the mean value for each cluster within each condition.  
