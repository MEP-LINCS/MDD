...
if(file.exists("R/assay_pk_data.rda")){
    load("R/assay_pk_data.rda")
} else {
  
md <- read_csv("metadata/MDD_sample_annotations.csv")

#read list of selected peaks to include in analysis
  ATACseq_selected_peaks <- read_csv("ATACseq/Data/MDD_peakList200.csv") %>%
  rename(feature = peak)
...
###ToDo merge with metadata and incorporate into analysis
...
assay_pk_data <- bind_rows(ATACseq_values, cycIF_values, GCP_values, hallmark_pathways, IF_values, RNA_values, RPPA_values, RPPA_pathways, TFs_values)

save(assay_pk_data,
     md,
     file = "R/assay_pk_data.rda") 
...
if(file.exists("R/selected_assay_pk_data.rda")){
    load("R/selected_assay_pk_data.rda")
...
selected_assay_pk_data <- bind_rows(ATACseq_selected, cycIF_values, GCP_selected, hallmark_pathways, IF_values, RNAseq_selected,  RPPA_selected, RPPA_pathways, TFs_selected)
...
save(selected_assay_pk_data,
     ...
     RPPA_pathways,
     TFs_selected,
     file = "R/selected_assay_pk_data.rda")
}

zscore_cutoff <- Inf
if(file.exists("R/selected_assay_pk_data_rr.rda")){
  load("R/selected_assay_pk_data_rr.rda")
  ...
  selected_assay_pk_data_rr <-  bind_rows(ATACseq_selected_rr, cycIF_values_rr, GCP_selected_rr,  hallmark_pathways_rr, IF_values_selected_rr, RNAseq_selected_rr,  RPPA_selected_rr, RPPA_pathways_rr, TFs_selected_rr)
  save(selected_assay_pk_data_rr, file = "R/selected_assay_pk_data_rr.rda")
}
...
df_as_matrix <-  prep_hm_matrix(selected_assay_pk_data_rr) %>%
  t %>%
  as.matrix() %>%
  scale %>%
  t
ann_df <- prep_hm_annotations(selected_assay_pk_data_rr)
