...
df_p_as_matrix <-  prep_hm_matrix(df_p, columns = "condition") %>%
  t %>%
  scale() %>%
  t
ann_df <- prep_hm_annotations(df_p, columns = "condition")

  colnames(df_p_as_matrix)[str_detect(colnames(df_p_as_matrix), "BMP2|IFNG|TGFB")] <- str_replace(colnames(df_p_as_matrix)[str_detect(colnames(df_p_as_matrix), "BMP2|IFNG|TGFB")], "_","_EGF_")
clust_num <- 7
...
cluster_groups <- kmeans(df_p_as_matrix, iter.max = 20, centers = centers)$cluster
...
hm_data <- tibble(feature = names(hm@plots@listData$Cluster@data),
  ...
  left_join(feature_gene, by = c("feature" = "feature"))
...
write_csv(df_p_sp, "MDD_condition.csv")
```
...
clusters <- lapply(1:clust_num, function(cl_select){
  ...
  res <- df_p_sp_sel %>%
    ...
    write_csv(paste0("MDD_condition_cluster_",cl_select,"_features.csv"))
  ...
  hm <- get_iheatmap(df_m, assay_name = "Selected assay data, rrscaled")%>%
    add_col_labels() %>%
...
write_csv(df_cor, "MDD_condition_correlations.csv")
...
for(assay in unique(df_UMAP$Type)){
...
df_UMAP_ligands <- colnames(df_UMAP)[str_detect(colnames(df_UMAP), "ctrl|PBS|HGF|OSM|EGF|BMP2|IFNG|TGFB")]
for(ligand in df_UMAP_ligands){
  ...
  scale_color_gradient2(low = "blue4", mid = "lavenderblush1",
  high = "red", midpoint = 0,) +
  labs()+
    guides(colour = FALSE)+
  # theme_bw()+
...
#Adding ctrl0 broke this gap statistic code
...
#   prep_hm_matrix(columns = "condition")
if(file.exists(paste0("R/gss_",cluster_method,"_condition_data.rda"))){
  load(paste0("R/gss_",cluster_method,"_condition_data.rda"))
} else {
  gss <- clusGap(df_p_as_matrix, FUN = kmeans, K.max = 17, B = 50, verbose = FALSE)
  save(gss, file = paste0("R/gss_",cluster_method,"_condition_data.rda"))
}
