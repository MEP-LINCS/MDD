...
write_csv(plsr_module_count, "MDD_intgrated_plsr_module_count.csv")
...
if(write_csvs){
  res <- lapply(1:clust_num, function(cl_select){
    df_p_sp_sel <- hm_data_ann %>%
      filter(Cluster == cl_select) %>%
      write_csv(paste0("Tables/Ligand_clusters/MDD_ligand_cluster_",cl_select,"_features",clust_num,".csv"),col_names = TRUE)
  })
}
