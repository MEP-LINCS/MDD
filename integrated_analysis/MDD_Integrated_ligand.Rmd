...
df_p_long <-  selected_assay_pk_data_rr %>%
  mutate(
    #feature = paste0(feature, "_", experimentalTimePoint),
         Type = str_replace(Type, "motifs", "motif"),
         ligand = str_replace(ligand, "ctrl", "CTRL")) %>%
...
df_p_T0 <- selected_assay_pk_data_rr %>%
...
assay_cols <- structure(RColorBrewer::brewer.pal(8, "Paired")[c(6, 2:3,5, 1, 7)],
...
chm <- Heatmap(df_p_as_matrix,
               name = "Z score",
               column_title = "Selected assay data, rrscaled, T0 relative",
...
chm
if(create_pdfs){
  pdf("MDD_ligands_heatmap_main_CHM_no_pathways_no_z_T0.pdf") 
  draw(chm)
  res <- dev.off()
} 
...
#Make a heatmap of the average cluster response
chm <- Heatmap(t(df_cl_as_matrix),
               name = "Z score",
               column_title = "MDD Integrated ligand cluster averages",
               column_title_gp = gpar(fontsize = 12),
               #col = colorRamp2(c(-1, 0, 1), c("#2166AC", "white", "#B2182B")),
               cluster_rows = FALSE,
               cluster_row_slices = FALSE, 
...
if(create_pdfs) {
  pdf("MDD_ligand_heatmap_cluster_averages.pdf")
  draw(chm)
  res <- dev.off()
} 
