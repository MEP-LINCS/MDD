...
if(file.exists("R/rrscale_assay.rda")){
  ...
  cycIF_values_rr <- rrscale_assay(cycIF_values)
...
```{r gapStatistics, fig.width=4, fig.height=4}
...
plot_gap = function(x) {
  ...
  ggplot(gstab, aes(k, gap)) + 
    ...
    labs(title = "Gap analysis to determine cluster number")
}
plot_gap(gss)
...
The first local maximum is k = 10. k = 11 is the first value with a gap value minus its standard error is less than the local maximum.  
