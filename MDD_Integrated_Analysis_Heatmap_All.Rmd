...
rrscale_assay <- function(df_long){
  df_sp <- df_long %>%
...
RPPA_pathways_rr <- rrscale_assay(RPPA_pathways)
...
hallmark_pathways_rr <- rrscale_assay(hallmark_pathways)
...
ATACseq_selected_rr <- rrscale_assay(ATACseq_selected)
...
ATACseq_motif_selected_rr <- rrscale_assay(ATACseq_motif_selected)
...
TFs_selected_rr <- rrscale_assay(TFs_selected)
...
RPPA_selected_rr <- rrscale_assay(RPPA_selected)
...
GCP_selected_rr <- rrscale_assay(GCP_selected)
...
RNAseq_selected_rr <- rrscale_assay(RNAseq_selected)
...
pk_selected_rr <-  bind_rows(ATACseq_selected_rr, ATACseq_motif_selected_rr, GCP_selected_rr, hallmark_pathways_rr, RNAseq_selected_rr, RPPA_pathways_rr, RPPA_selected_rr, TFs_selected_rr)
...
if(length(rrscale_file)==0){
  if(rrscale_small){
    ...
    all_rr_values <- map(df_as_list_small, rrscaleValues)
    save(all_rr_values, file = "R/rrscale.rda")
  } else{
    all_rr_values <- map(df_as_list, rrscaleValues)
    save(all_rr_values, file = "R/rrscale.rda")
  }
...
# condition_order <-  c("ctrl_0",paste(rep(c("PBS", "HGF", "OSM", "EGF","BMP2", "IFNG", "TGFB"), each = 2), rep(c(24, 48),  times = 7),   sep = "_"))
# 
# df_sp <-  pk_top_rr %>%
#   dplyr::select(-value) %>%
#   rename(value = value_rr) %>%
#   spread(key = experimentalCondition, value = value) %>%
#   filter(!is.na(Type)) %>%
#   dplyr::select(feature, Type, condition_order)
# 
# df_mat <- df_sp %>%
...
#cluster analysis 
#calinhara(hm@plots@listData$score@data, hm@plots@listData$Cluster@data)
# CH <- tibble(
#   k = 2:20,
#   value = sapply(k, function(i) {
#     hm <- get_iheatmap(df_as_matrix, assay_name = "Prior knowledge, rrscale-transformed", k = i)
#     foo <- hm@plots@listData$score@data %>%
#       as.data.frame() %>%
#       mutate(Cluster = hm@plots@listData$Cluster@data) %>%
#       drop_na()
#     ch_value <- calinhara(select(foo, -Cluster), foo$Cluster)
#     return(ch_value)
#   })
# )
# 
# p <- ggplot(CH, aes(x = k, y = value)) +
#   geom_line() +
#   geom_point() +
#   labs(y = "Calinski-Harabsz index")
# p
...
###Todo continue with code from page 126
df_as_matrix <- df %>%
  prep_hm_matrix
if(file.exists("R/gss.rda")){
  load("R/gss.rda")
} else {
  gss <- clusGap(df_as_matrix, FUN = hafun, K.max = 20, B = 100, verbose = FALSE)
  save(gss, file = "R/gss.rda")
}
...
This dataset combines MCF10A Molecular Deep Dive (MDD) RNAseq, Reverse Phase Protein Array(RPPA), Global ChromatinCProfiling (GCP) histone modifications, immunofluorescence (IF), cyclic immunofluorescence (cycIF) and ATACseq chromatin configuration data with prior knowledge of signaling pathways, DNA motifs and transcription factors. All data is collected at 0, 24 and 48 hours. Most of the data is median summarized from three replicate experiments run in sequential weeks. 

RRscale  
Use rrscale https://CRAN.R-project.org/package=rrscale to transform each feature in the heatmap before clustering. This method performs a box-cox gaussianizing transformation on each feature, zscores the results then eliminates outliers with absolute values greater than 4.  
...
Clustering  
The hierarchical clustering uses the complete linkage method to find similar clusters. The number of clusters as shown in the side annotation bar. It is chosen as `r hclust_clust_num` based on gap statistic analysis. The scatter plot above the heatmap shows the mean value for each cluster within each condition.  
...
```
```{r befopreAfterrr, fig.height=12}
...
df <- pk_top_rr %>%
  ...
  ungroup() %>%
  drop_na() %>%
  inner_join(pk_top_rr, by = c("feature", "Type")) %>%
  arrange(Type)
