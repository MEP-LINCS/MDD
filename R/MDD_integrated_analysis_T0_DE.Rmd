---
title: "MDD integrated data analysis by condition using z scores and PAM clustering"
...
data_path <- "../integrated_analysis/integrated_matrix_lfc_rr.csv"
p_values_path <- "../integrated_analysis/integrated_adj_p_values.csv"
shared_features_path <- "../integrated_analysis/integrated_shared_features.csv"
unique_features_path <- "../integrated_analysis/integrated_unique_features.csv"
...
integrated_T0_DE_all <- read_csv(data_path) %>%
...
integrated_unique <- read_csv(unique_features_path) %>%
  ...
  select(ligand, Type, feature) %>%
  left_join(integrated_T0_DE_all, by = c("feature" = "feature", "Type" = "Type" )) %>%
  rename(Cluster = ligand) %>%
  drop_na

#Read in the integrated data, add EGF to some names, set order
if(!file.exists(shared_features_path)) stop("cannot read ",shared_features_path)
integrated_shared <- read_csv(shared_features_path) %>%
  mutate(ligand = str_replace(ligand, "BMP2","BMP2+EGF"),
         ligand = str_replace(ligand, "IFNG","IFNG+EGF"),
         ligand = str_replace(ligand, "TGFB","TGFB+EGF")) %>%
  select(Type, feature) %>%
...
# #Read in adjusted p values and use to filter
# adj_p_value_threshold <- .01
# evaluate_p_values <- function(x){
#   any(x <= adj_p_value_threshold)
# }
# integrated_adj_p_values <- read_csv(p_values_path,
#                                     col_types = cols(
#                                       Type = col_character(),
...
#   drop_na() %>%
#   rowwise() %>%
#   filter(evaluate_p_values(c_across(PBS_24:EGF_48)))
...
assay_cols <- structure(RColorBrewer::brewer.pal(8, "Paired")[c(6, 2:3,5, 1)],
                        names = unique(integrated_shared$Type))

#Create data and metadata matrices and annotations
#k means cluster and order by cluster value

int_T0_DE_matrix <- integrated_shared %>%
  select(all_of(condition_order)) %>%
  as.matrix() 

rownames(int_T0_DE_matrix) <- integrated_shared$feature
...
clusters <- kmeans(x = int_T0_DE_matrix, iter.max = 20, centers = centers)$cluster
  ...
  clust_obj <- hclust(dist(int_T0_DE_matrix))
  clusters <- cutree(clust_obj, k = clust_num)
} else if(cluster_method == "pam") {
  clusters  <- pam(x = int_T0_DE_matrix, k = clust_num, cluster.only=TRUE)
} else stop("unsupported cluster method chosen")

integrated_shared<-tibble(feature = names(clusters),
                 Cluster = clusters) %>%
  right_join(integrated_shared, by = "feature") %>%
  arrange(desc(Cluster))

heatmap_ann <- integrated_shared %>%
  select(Cluster, Type) %>%
...
hm <- Heatmap(dm,
               ...
               row_split = integrated_shared$Cluster,
               row_gap = unit(2, "mm"),
               row_title = " ",
...
hm
if(create_pdfs){
  pdf("MDD_integrated_rr_heatmap.pdf") 
  draw(hm)
  res <- dev.off()
} 

#Create a matrix and heatmap of the mean cluster values by condition
 integrated_T0_DE_mean <- integrated_shared %>%
  group_by(Cluster) %>%
...
hm_unique_ann <- integrated_unique %>%
  select(Cluster, Type) %>%
  mutate(Cluster = factor(Cluster, ordered = TRUE)) %>%
  as.data.frame()


haRow_unique <- HeatmapAnnotation(df = hm_unique_ann,
                                  which = "row",
                                  col = list(Cluster = factor(ligand_cols, levels = ligand_cols, ordered = TRUE),
                                             Type = assay_cols))

dm_unique <- integrated_unique %>%
    select(all_of(condition_order)) %>%
  as.matrix()

hm_unique <- Heatmap(dm_unique,
               name = "lfc rr",
               ...
               cluster_row_slices = FALSE, 
               cluster_columns = FALSE,
               row_split = integrated_unique$Cluster,
               row_gap = unit(2, "mm"),
               row_title = " ",
               show_row_names = FALSE,
               row_names_gp = gpar(fontsize = 8),
...
module_features <- integrated_shared %>%
...
module_features <- integrated_unique %>%
  arrange(Cluster, Type) %>%
  mutate(Cluster = paste0("module_", Cluster),
         Cluster = factor(Cluster, levels = unique(Cluster))) %>%
  split(.$Cluster) 
...
dm <- integrated_shared %>%
  ...
  as.matrix()
rownames(dm) <- integrated_shared$feature
...
if_module_dm <- rbind(dm_mean, dm_unique_mean, if_dm) %>%
  t
