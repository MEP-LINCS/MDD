...
Create include all ligands and a no PBS set  
...
KEGG_dm_noPBS <- KEGG_dm[,!str_detect(colnames(KEGG_dm),"PBS")]
hm_KEGG_noPBS <- Heatmap(KEGG_dm_noPBS,
                       name = "NES",
                       column_title = "KEGG GSEA in MDD RNAseq",
                       column_title_gp = gpar(fontsize = 12),
                       #col = colorRamp2(c(-2, 0, 2), c("#2166AC", "white", "#B2182B")),
...
res <- pdf("../plots/MDD_GSEA_KEGG.pdf",width = 5, height = 8)
draw(hm_KEGG)
draw(hm_KEGG_noPBS)
res <- dev.off()

write_csv(as.data.frame(KEGG_dm_noPBS) %>%
  mutate(gene = rownames(KEGG_dm_noPBS)) %>%
  select(gene, everything()), "../tables/MDD_RNAseq_noPBS_KEGG_GSEA.csv")
