...
load_data <- function(assay_name, level = 4){
  ...
  df_g <- gather(df, key = "specimenID", value = "value", -feature)
  return(df_g)
} #end of function
...
l3 <- read_csv("../Metadata/MDD_sample_annotations.csv") %>%
  select(specimenID, specimenName, ligand, experimentalCondition, secondLigand, experimentalTimePoint, collection, replicate) %>%
...
p <- ggplot(df, aes(x=experimentalCondition, y=WellCellCountT0Norm, fill=Ligand))+
  geom_boxplot(position = position_dodge(width=0.9), lwd = .5, outlier.colour = NA)+
...
p <- p +  geom_point(aes(shape=collection), color = "black", size = 3, position=position_jitterdodge(dodge.width=0.9), alpha=.8)
