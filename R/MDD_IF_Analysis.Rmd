...
df <- l3_T0 %>%
  filter(experimentalTimePoint %in% c(0,24))

p <- ggplot(df, aes(x=Ligand, y=WellCellCountT0Norm, fill=Ligand, color = Ligand2))+
  geom_boxplot(position = position_dodge(width=0.9), lwd = 1.1, outlier.colour = NA)+
  scale_fill_manual(values = ligand_cols) +
  scale_color_manual(values = ligand2_cols) +
  coord_cartesian(y = c(0,9)) +
  scale_y_continuous(breaks = c(1,3,5,7,9)) +
  labs(x="Ligand",
       y="Well Cell Count",
       title = paste("Time 0 and 24"))+
  theme_bw() +
