...
## Libraries ordered by # of groups with ≥1 significant TF

```{r}
temp <- unique(listLongFDR$`Query Name`)
...
## Summary of significant CHEA3 TFs - FET.p.value threshold

For each `r FEATURETYPE`, all genes were submitted to [CHEA3](https://maayanlab.cloud/chea3/) via
their API. Results from all TF libraries were returned. Significant TFs were identified by selecting those
with a p-value < `r FDR_THRESHOLD`.
...
listLongFET <-
  ...
  mutate(FDR = as.numeric(FDR),
         `FET p-value` = as.numeric(`FET p-value`),
         Rank = as.numeric(Rank))
```
...
```{r, fig.width=8}
listLongFET %>%
  filter(`FET p-value` < FDR_THRESHOLD, .preserve = TRUE) %>%
  group_by(`Query Name`, Library) %>%
  summarize(n = n()) %>%
  ggplot(aes(`Query Name`, n)) +
  geom_col(position = "dodge") +
...
```{r}
temp <- unique(listLongFET$`Query Name`)
...
```{r, warning=FALSE}
listLongFET %>%
  filter(`FET p-value` < FDR_THRESHOLD) %>%
  arrange(`FET p-value`) %>%
  dplyr::select(-Score, -Overlapping_Genes) %>%
  datatable
```
