...
#pathway_nbr <- 3
#                          
# res <- make_dotplots_API(pathways_combined14,
#                          pathway_method = pathway_method,
#                          fdr_thresh = fdr_thresh,
#                          gene_ratio_thresh = gene_ratio_thresh,
#                          min_pathway_genes = min_pathway_genes,
...
gene_pathways <- map2(module9_dotplot_genes$stId, module9_dotplot_genes$submitted_entities_found, parse_entities) %>%
  ...
  select(gene, stId, module, name, superpathway)
...
gene_pathways_module9 <- map2(module9_reactome_web$pathway_identifier, module9_reactome_web$submitted_entities_found, parse_entities) %>%
  bind_rows() %>%
  distinct()  %>%
  left_join(pathway_names, by = "stId")

res <- write_csv(gene_pathways_module9, "../tables/MDD_integrated_reactome_module9_genes_pathways.csv")
#Mock up a circos plot for module 9.
#connect the 3 significant pathways to their highest  superpathway.

#Manually annotate pathways with highest superpathway
