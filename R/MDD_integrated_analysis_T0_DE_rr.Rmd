...
Use replicates to perform differential (DE) analysis between conditons and ctrl_0 for RNAseq, RPPA, GCP and cycIF assays. Use family z scores for motifs.  
...
#Remove the cluster assignements to keep the code similar to the 
...
hm_combined14_mean_clustered_obj <- Heatmap(combined14_features_data_mean_dm,
                            name = "lfc ",
                            column_title = "combined14 cluster means",
                            column_title_gp = gpar(fontsize = 12),
                            col = colorRamp2(c(-2, 0, 2), c("#2166AC", "white", "#B2182B")),
                            ...
                            column_names_gp = gpar(fontsize = 10),
                            use_raster = FALSE)
hm_combined14_mean_clustered <- draw(hm_combined14_mean_clustered_obj)
combined14_cluster_order <- row_order(hm_combined14_mean_clustered)
...
if(create_pdfs){
  ...
  draw(hm_combined14_mean_clustered)
  draw(hm_combined14)
...
hm_unique_RPPA_48 <- Heatmap(unique_features_RPPA_48_dm,
                     name = "lfc rr",
                     column_title = "Ligand-specific RPPA 48H",
                     column_title_gp = gpar(fontsize = 12),
                     col = colorRamp2(c(-2, 0, 2), c("#2166AC", "white", "#B2182B")),
                     cluster_rows = FALSE,
                     cluster_row_slices = FALSE, 
...
hm_unique_motifs_48 <- Heatmap(unique_features_motifs_48_dm,
                     name = "lfc rr",
                     column_title = "Ligand-specific motifs 48H",
                     column_title_gp = gpar(fontsize = 12),
                     col = colorRamp2(c(-2, 0, 2), c("#2166AC", "white", "#B2182B")),
                     cluster_rows = FALSE,
                     cluster_row_slices = FALSE, 
...
hm_unique_cycIF_48 <- Heatmap(unique_features_cycIF_48_dm,
                     name = "lfc rr",
                     column_title = "Ligand-specific cycIF 48H",
                     column_title_gp = gpar(fontsize = 12),
                     col = colorRamp2(c(-2, 0, 2), c("#2166AC", "white", "#B2182B")),
                     cluster_rows = FALSE,
                     cluster_row_slices = FALSE, 
...
# ####
if(create_pdfs){
  pdf("../plots/MDD_integrated_rr_unique_heatmap.pdf")
  draw(hm_unique)
...
# ####
if(create_pdfs){
  pdf("../plots/../plots/MDD_integrated_rr_unique_heatmap_ordered.pdf")
  draw(hm_noRNAseq)
  draw(hm_GCP)
  draw(hm_cycIF)
  draw(hm_RPPA)
...
if(create_pdfs){
  pdf("../plots/../plots/MDD_unique_heatmaps.pdf") 
  draw(hm_unique)
  draw(hm_unique_mean)
  res <- dev.off()
} 
...
hm_T0 <- Heatmap(df_dm,
               name = "lfc rr",
               column_title = "Ligand-specific features of cycIF, GCP, RPPA",
               column_title_gp = gpar(fontsize = 12),
               col = colorRamp2(c(-2, 0, 2), c("#2166AC", "white", "#B2182B")),
               cluster_rows = FALSE,
               cluster_row_slices = FALSE,
...
p_list <- lapply(df_UMAP_conditions, function(condition){
  ...
  highlights <- df %>%
    filter(Set == ligand,
           feature_type %in% c("Stat3_pY705_RPPA")) 
...
p
if(create_pdfs){
  pdf("../plots/MDD_combined14_cluster_feature_counts.pdf", width = 4, height = 3)
    print(p_hist)
  res <- dev.off()
} 
...
if(create_pdfs){
  pdf("../plots/MDD_combined14_cluster_mean_bars.pdf", width = 4, height = 3)
  print(p_cluster)
  print(p_ligand)
  res <- dev.off()
} 
...
cycIF_combined18_features_int <- combined18_features_data_ann %>%
  filter(Type == "cycIF",
...
###DEBUG problem in this section
#T0 cycIF features were average of intenisty values in nuc and cytoplasm
#The T0 DE cycIF values are indivdual compartment intensities with assigned clusters
#the T0 values don't map to the DE features so clusters can not be assigned
#some cyto vs nuc are in different clusers
combined18_features_with_cycIF_int <- c(combined18_features_data_ann$feature_type, cycIF_combined18_features_int$feature_type)
...
matched_features_types <- assay_pk_data %>%
    mutate(Type = case_when(Type == "RNA" ~"RNAseq",
           ...
           condition = paste0(ligand,"_", experimentalTimePoint)) %>%
  select(-ligand, -feature, -Type, -experimentalTimePoint) %>%
  filter(feature_type %in% combined18_features_with_cycIF_int)
...
#create a numeric matrix with feature row names
assay_pk_data_T0_norm_dm <- assay_pk_data_T0_norm_24_48 %>%
  select(-feature_type, -Cluster) %>%
  as.matrix() 

rownames(assay_pk_data_T0_norm_dm) <- assay_pk_data_T0_norm_24_48$feature_type
# combined18_features_dm <- combined18_features_dm[!is.nan(combined18_features_dm[,1]),]
...
hm_T0 <- Heatmap(assay_pk_data_T0_norm_dm,
        ...
        cluster_row_slices = FALSE,
        cluster_columns = FALSE,
        row_split = assay_pk_data_T0_norm_24_48$Cluster,
        #row_gap = unit(2, "mm"),
        row_title = " ",
...
assay_pk_data_T0_norm_mean <- assay_pk_data_T0_norm_24_48 %>%
...
#and drop RNAseq
cluster14_assignments <- combined14_features_data_ann %>%
  select(Cluster, feature_type)
...
assay_pk_data_T0_norm14 <- assay_pk_data_T0_norm %>%
  select(-Cluster) %>%
  left_join(cluster14_assignments, by = "feature_type") %>%
  filter(!str_detect(feature_type, "RNAseq")) %>%
  select(ligand_order_all_times,feature_type, Cluster)
...
assay_pk_data_T0_norm14_mean <- assay_pk_data_T0_norm14 %>%
  ...
  summarise(value_mean =mean(value)) %>%
  pivot_wider(names_from = condition, values_from = value_mean) %>%
  select(all_conditions_without_1, Cluster)
