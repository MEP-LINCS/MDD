...
shared_features_data <- shared_features %>%
  ...
  drop_na
set.seed(42)
combined_features_data <- bind_rows(shared_features_data, select(unique_features_data, -Direction, -Cluster)) %>%
  slice_sample(prop = 1)
assay_cols <- structure(RColorBrewer::brewer.pal(8, "Paired")[c(3, 2,6,5, 1)], names = unique(combined_features_data$Type))
...
combined_features_dm <- combined_features_data %>%
  select(all_of(condition_order)) %>%
  as.matrix() 
rownames(combined_features_dm) <- combined_features_data$feature_type
combined_features_dm <- combined_features_dm[!is.nan(combined_features_dm[,1]),]

#cluster all features 
if(cluster_method == "kmeans"){
  #seed the clustering with a fixed, random set of points
  set.seed(42)
  centers <- combined_features_dm[sample(1:nrow(combined_features_dm), size = combined_clust_num, replace = FALSE),]
  clusters <- kmeans(x = combined_features_dm, iter.max = 20, centers = centers)$cluster
} else if(cluster_method == "HA") {
  clust_obj <- hclust(dist(combined_features_dm))
  ...
  if(file.exists(paste0("../integrated_analysis/MDD_",cluster_method,"_clusters.rda"))){
    ...
    set.seed(42)
    medoids <- sample(1:nrow(combined_features_dm), size = combined_clust_num, replace = FALSE)
    clusters  <- pam(x = combined_features_dm, k = combined_clust_num, medoids = medoids, cluster.only=TRUE, pamonce = 5)
...
combined_features_data_ann <-  tibble(feature_type = names(clusters),
                           Cluster = clusters) %>%
  right_join(combined_features_data, by = "feature_type") %>%
    mutate(Cluster = factor(Cluster, ordered = TRUE))

combined_features_ann <- combined_features_data_ann %>%
  select(Cluster, Type, Set) %>%
  as.data.frame()

#Create a matrix and heatmap of the mean combined values
 combined_features_data_mean <- combined_features_data_ann %>%
  group_by(Cluster) %>%
  summarise(across(.cols = matches("_24|48"), .fns = mean))

  combined_features_count <- combined_features_data_ann %>%
  group_by(Cluster) %>%
 count()
  
combined_features_data_mean_dm <- combined_features_data_mean %>%
  select(-Cluster) %>%
  as.matrix()
rownames(combined_features_data_mean_dm) <- paste0("Module_",combined_features_data_mean$Cluster,"_", combined_features_count$n)

hm_combined_mean <- Heatmap(combined_features_data_mean_dm,
               name = "lfc ",
               column_title = "Combined cluster means",
...
haRow_combined <- HeatmapAnnotation(df = combined_features_ann,
                               which = "row",
                               col = list(Cluster = factor(cluster_cols, levels = cluster_cols, ordered = TRUE),
                                          Type = assay_cols,
                                          Set = c("shared" = "black", ligand_cols)))

hm_combined <- Heatmap(combined_features_dm,
               name = "lfc rr",
               column_title = "Combined features",
               column_title_gp = gpar(fontsize = 12),
               col = colorRamp2(c(-1.5, 0, 1.5), c("#2166AC", "white", "#B2182B")),
               cluster_rows = FALSE,
               cluster_row_slices = FALSE, 
               cluster_columns = FALSE,
               row_split = combined_features_ann$Cluster,
               row_gap = unit(2, "mm"),
               row_title = " ",
               show_row_names = FALSE,
               row_names_gp = gpar(fontsize = 8),
               column_names_gp = gpar(fontsize = 10),
               left_annotation = haRow_combined,
               use_raster = FALSE)
hm_combined

if(create_pdfs){
  pdf("MDD_integrated_rr_combined_heatmap.pdf") 
  draw(hm_combined)
  res <- dev.off()
} 
...
####
#Create a matrix and heatmap of the mean unique cluster values
...
module_features <- combined_features_data_ann %>%
...
res <- write_xlsx(module_features,
                  path = "MDD_int_rr_combined_tables.xlsx")

##### Randomized cluster assignments for null set analysis
module_features_null <- combined_features_data_ann %>%
    mutate(Cluster = Cluster[sample(nrow(combined_features_data_ann))]) %>%
  arrange(Cluster, Type) %>%
  mutate(Cluster = paste0("module_", Cluster),
         Cluster = factor(Cluster, levels = unique(Cluster))) %>%
  split(.$Cluster) 
...
res <- write_xlsx(module_features_null,
                  path = "MDD_int_rr_combined_tables_null.xlsx")
#####
...
dm <- combined_features_data_ann %>%
  dplyr::select(-feature, -Cluster, -Type, -feature_type, -Set) %>%
  as.matrix()
rownames(dm) <- combined_features_data_ann$feature_type
...
if(file.exists(paste0("../integrated_analysis/MDD_",cluster_method,"_combined_UMAP.rda"))){
  load(paste0("../integrated_analysis/MDD_",cluster_method,"_combined_UMAP.rda"))
} else {
df_UMAP <- umap(dm, config = custom_settings, na.rm = TRUE)$layout %>%
  data.frame(feature_type = rownames(dm)) %>%
  rename(UMAP_1 = X1,
         UMAP_2 = X2) %>%
  left_join(combined_features_data_ann, by = "feature_type") 
save(df_UMAP, file = paste0("../integrated_analysis/MDD_",cluster_method,"_combined_UMAP.rda"))
}
...
if(plot_UMAP_3d){
  ...
  p <- ggplot(df_UMAP, aes(x = UMAP_1,
    ...
    labs(title = paste("UMAP embedding colored by cluster, combined"),
         colour = "Cluster") +
  ...
  if(create_pdfs) {
    pdf("MDD_combined_umap_main.pdf")    
    print(p)
    res <- dev.off()
  } 
```

There are `r dim(dm)[1]` dots in the 2d UMAP that represent the significant differentially expressed features relative to T0 in the combined-features integrated matrix.  
...
p <- ggplot(data = df_UMAP[df_UMAP$Type=="RNA",], aes(x = UMAP_1,
  ...
  labs(title = paste("UMAP embedding colored by assay, combined"),
       colour = "Assay") +
  guides(colour = guide_legend(override.aes = list(alpha = 1, size = 4))) +
  theme_bw()+
  theme(axis.title = element_blank(),
...
if(create_pdfs) {
  pdf("MDD_ligands_combined_umap_type.pdf") 
      print(p)
  res <- dev.off()
}
...
if(create_pdfs) {
  pdf("MDD_ligands_combined_umap_type_highlights.pdf") 
  res <- map(p_list, print)
  res <- dev.off()
}
```
...
res <- map(p_list, print)
if(create_pdfs) {
  pdf("MDD_integrated_combined_umap_by_condition.pdf")
  res <- map(p_list, print)
  res <- dev.off()
}
...
# combined_features_data_mean_df <- combined_features_data_ann %>%
#   group_by(Cluster) %>%
#       summarise(across(.cols = matches("_24|48"), .fns = mean)) %>%
#     ungroup() %>%
#   mutate(Cluster = paste0("Module_",as.integer(Cluster)))
# combined_features_data_mean_dm <- combined_features_data_mean_df %>%
#   select(-Cluster) %>%
#   as.matrix()
# rownames(combined_features_data_mean_dm) <- combined_features_data_mean_df$Cluster
  
if_module_dm <- rbind(combined_features_data_mean_dm, if_dm) %>%
  t
...
if(create_pdfs) {
  pdf("MDD_combined_IF_correlations.pdf")
  res <- print(cor_hm)
  res <- dev.off()
}
...
if(create_pdfs) {
  pdf("MDD_combined_IF_correlations_reduced.pdf",
      width = 9, height = 8)
  res <- print(cor_hm)
  res <- dev.off()
}
...
if(create_pdfs) {
  pdf("MDD_combined_module_module_correlations_reduced.pdf",
      width = 9, height = 8)
  res <- print(cor_hm)
  res <- dev.off()
}
if(write_csvs) {
  write.csv(cor_dm, "MDD_combined_module_module_correlations_reduced.csv")
...
if(create_pdfs) {
  pdf("MDD_combined_module_phenotype_correlations_reduced.pdf",
      width = 9, height = 8)
  res <- print(cor_hm)
  res <- dev.off()
}
...
#create a bar chart of the combined feature types
df <- combined_features_data %>%
  #drop_na() %>%
  select(feature, Type) %>%
  distinct()
...
p <- ggplot(df, aes(x = Type, fill = Type)) +
  ...
  coord_cartesian(ylim = c(0, 10000)) +
  scale_fill_manual(values = assay_cols)+
  labs(title = paste("Type distribution of the",length(unique(paste0(df$feature))), "features at unique timepoints in the combined-features MDD integrated dataset"),
       x ="Feature type") +
  theme_bw()+
  theme(axis.text.x =  element_text(angle = 90),
    axis.ticks = element_blank(),
...
p_log <- ggplot(df, aes(x = Type, fill = Type)) +
  ...
  scale_y_log10() +
  scale_fill_manual(values = assay_cols)+
  labs(title = paste("Type distribution of the",length(unique(paste0(df$feature))), "combined features at unique timepoints in the MDD integrated dataset"),
       x ="Feature type") +
  theme_bw()+
  theme(axis.text.x =  element_text(angle = 90),
    axis.ticks = element_blank(),
...
p_log
if(create_pdfs){
  pdf("MDD_combined_feature_type_distribution.pdf")
  print(p)  
  res <- dev.off()
  pdf("MDD_combined_feature_type_distribution_log_scale.pdf")
  print(p_log)
  res <- dev.off()
} 
...
dm <- combined_features_data %>%
    select(all_of(condition_order)) %>%
  as.matrix()

if(file.exists(paste0("../integrated_analysis/gss_",cluster_method,"_combined.rda"))){
  load(paste0("../integrated_analysis/gss_",cluster_method,"_combined.rda"))
} else {
  gss_combined <- clusGap(dm, FUN = pam, pamonce = 5, K.max = 25, B = 100, verbose = FALSE)
  save(gss_combined, file = paste0("../integrated_analysis/gss_",cluster_method,"_combined.rda"))
}

  plot_gap_custom(gss_combined, "combined")
if(create_pdfs){
  pdf("MDD_combined_gap_cluster.pdf")
  p <- plot_gap_custom(gss_combined)
  print(p)
  res <- dev.off()
}
...
p_hist <- ggplot(combined_features_data_ann, aes(x = as.factor(Cluster), fill = Type)) +
  geom_bar(position = "dodge") +  
  scale_fill_manual(values = assay_cols) +
  labs(title = "Combined cluster analysis",
       x = "Cluster",
       fill = "Assay") +
  facet_wrap(~Type, scales = "free_y") +
  theme_bw()+
...
if(create_pdfs){
  pdf("MDD_combined_cluster_feature_counts.pdf")
     print(p_hist)
     print(p)
  res <- dev.off()
} 
...
"Tibs2001SEmax": `r maxSE(gss_combined$Tab[,'gap'], SE.f = gss_combined$Tab[,'SE.sim'],
...
"firstSEmax": `r maxSE(gss_combined$Tab[,'gap'], SE.f = gss_combined$Tab[,'SE.sim'],
      method = c("firstSEmax"),
      SE.factor = 1) `  
...
"globalSEmax": `r maxSE(gss_combined$Tab[,'gap'], SE.f = gss_combined$Tab[,'SE.sim'],
...
Three of the methods for choosing k using a gap statitistic are:  
"Tibs2001SEmax": `r maxSE(gss_shared$Tab[,'gap'], SE.f = gss_shared$Tab[,'SE.sim'],
      method = c("Tibs2001SEmax"),
      SE.factor = 1) `  
...
"firstSEmax": `r maxSE(gss_shared$Tab[,'gap'], SE.f = gss_shared$Tab[,'SE.sim'],
      method = c("firstSEmax"),
      SE.factor = 1) `  
...
"globalSEmax": `r maxSE(gss_shared$Tab[,'gap'], SE.f = gss_shared$Tab[,'SE.sim'],
      method = c("globalSEmax"),
      SE.factor = 1)`    
...
set_genes <- combined_features_data_ann %>%
  filter(Type == "RNAseq") %>%
...
pdf("combined_comparisons.pdf", width = 10, height = 8)
upset_obj
res = dev.off()
...
matched_features_types <- assay_pk_data %>%
  ...
  filter(feature_type %in% combined_features_data_ann$feature_type,
...
if(create_pdfs){
  pdf("MDD_T0_normed_in_combined_manual_clusters.pdf",height = 10)
  p
  draw(hm_T0)
  draw(hm_T0_mean)
  res <- dev.off()
...
res <- write_xlsx(module_features,
                  path = "MDD_T0_norm_combined_tables.xlsx")
