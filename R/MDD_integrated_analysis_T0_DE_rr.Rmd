...
cluster_cols <- c(structure(RColorBrewer::brewer.pal(12, "Paired")[c(1:12)], names = 1:12), c("13" = "black", "14" = "gray", "15" = "brown", "16" = "gray20", "17" = "gray40", "18" = "gray60", "19" = "gray80"))
...
if(file.exists(paste0("../integrated_analysis/MDD_",cluster_method,"_shared_UMAP.rda"))){
  load(paste0("../integrated_analysis/MDD_",cluster_method,"_shared_UMAP.rda"))
} else {
...
save(df_UMAP, file = paste0("../integrated_analysis/MDD_",cluster_method,"_shared_UMAP.rda"))
}
...
if(create_pdfs) {
  pdf("MDD_ligands_shared_umap_type.pdf") 
      print(p)
  res <- dev.off()
}
...
if(create_pdfs) {
  pdf("MDD_ligands_shared_umap_type_highlights.pdf") 
  res <- map(p_list, print)
  res <- dev.off()
}
```
...
res <- map(p_list, print)
if(create_pdfs) {
  pdf("MDD_integrated_shared_umap_by_condition.pdf")
  res <- map(p_list, print)
  res <- dev.off()
}
...
if(file.exists(paste0("../integrated_analysis/gss_",cluster_method,"_combined_B100.rda"))){
  load(paste0("../integrated_analysis/gss_",cluster_method,"_combined_B100.rda"))
} else {
...
if(file.exists(paste0("../integrated_analysis/gss_",cluster_method,"_shared_B100.rda"))){
  load(paste0("../integrated_analysis/gss_",cluster_method,"_shared_B100.rda"))
} else {
  gss_shared <- clusGap(dm, FUN = pam, pamonce = 5, K.max = 25, B = 100, verbose = FALSE)
  save(gss_shared, file = paste0("../integrated_analysis/gss_",cluster_method,"_shared_B100.rda"))
}

  plot_gap_custom(gss_shared)
if(create_pdfs){
