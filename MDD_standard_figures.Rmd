...
plot_circos <- function(df, md, assay_name, ligand_cols, ligand_2_cols) {
  ####DEBUG refactor to match new data sturcture######
 df_matrix = select(df, -feature) %>%
            as.matrix() %>%
  ...
  res <- draw_one_track(df, md, title_text = "RPPA Correlations")
  df <- rppa
  for(i in 1:nrow(df)){
    foo <- parseCorMatrix(df[i,])
    res <- apply(foo, 1, addLink)
...
```{r}
var_thresh_percentile <- 0.1

plot_heatmap(RPPA_l1, md, assay_name = "RPPA", EGF_normed = FALSE, var_thresh_percentile = var_thresh_percentile, mark_rows = c("EGFR", "Smad3","HER2", "HER3", "Stat3", "Claudin-7", "YAP", "Notch1"))

plot_heatmap(RPPA, md, assay_name = "RPPA", var_thresh_percentile = var_thresh_percentile, mark_rows = c("EGFR", "Smad3","HER2", "HER3", "Stat3", "Claudin-7", "YAP", "Notch1"))

```

The top `r var_thresh_percentile*100` percentile varying proteins measured in the RPPA assay are shown in the heatmaps.  
...
The unnormalized values are row-median centered. 
