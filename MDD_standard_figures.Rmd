...
plot_heatmap <- function(df, md, assay_name, EGF_normed = TRUE, var_thresh_percentile = 0, show_row_names = FALSE, mark_rows = NULL, cluster_rows = TRUE, select_timepoint = FALSE){
...
title_suffix <- ""
  ...
  if(select_timepoint){
    df <- df %>%
    select(matches(paste0("_",select_timepoint,"_","|feature")))
  }
...
cycIF <- prep_data(cycIF, md = md) %>%
  ...
  ungroup() %>%
    left_join(md) %>%
  select(feature, specimenID, ZScore) %>%
...
plot_heatmap(cycIF_hi_vars, md, assay_name = "cycIF", EGF_normed = FALSE)

```


Intensity features included in the heatmap have a variance above the `r var_thresh_percentile` percentile . Feature values are median centered within their replicate set. 
...
GCP_level2 <- prep_data(GCP_level2, md = md) %>%
  gather(specimenName, value, -feature) %>%
  mutate(Replicate = str_remove(specimenName, ".*_")) %>%
  group_by(Replicate, feature) %>%
  mutate(MedCenter = value-median(value, na.rm = TRUE),
...
plot_heatmap(ATACseq_hi_vars, md, EGF_normed = FALSE, assay_name = "ATACseq")
...
df <- L1000
  ...
  df_UMAP <- umap(df_mat, na.rm = TRUE, n_epochs = 5000, min_dist = .01, n_neighbors = 15, random_state = 42)$layout %>%
  data.frame() %>%
  rename(UMAP_1 = X1,
         UMAP_2 = X2) %>%
    cbind(df_ann) %>%
...
p <- ggplot(df_UMAP, aes(x = UMAP_1,
  ...
  geom_point(shape = 21, size = 4, alpha = .8) +
  scale_color_manual( values = ligand_cols) +
  scale_fill_manual( values = ligand_2_cols) +
  ...
  theme_bw()
p
pdf("L1000_T48_UMAP.pdf", width = 9, height = 8)
print(p)
dev.off()
```
