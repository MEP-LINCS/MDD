...
#   rename(feature = ensembl_gene_id)
 ...
 RNAseq <- read_csv("RNAseq/Data/MDD_RNAseq_Level3_HGNC.csv") %>%
   drop_na() %>%
   group_by(specimenID, feature) %>%
...
RNAseq_hi_vars <- RNAseq %>%
  inner_join(RNAseq_variance_genes, by = c("feature" = "hgnc_symbol"))
...
Genes included in the heatmap are the top 200 high variance genes with significant fold changes identified in a DE analysis of each ligand.  
...
This UMAP is based on all genes in the RNAseq dataset
...
The PCA is based on all genes in the RNAseq dataset
...
Spearman (rank-based) correlations using all available pairwise data points in the hi vars version of the RNAseq dataset.  
...
```{r getChEA3TFs}
#read in TF scores
getTFs <- function(x, logFC_thresh = 1){
  library(httr)
  library(jsonlite)
  genes <- read_tsv(paste0("./RNAseq/Data/",x)) %>%
    filter(abs(logFC) > logFC_thresh) %>%
...
if(!file.exists("RNAseq/Data/ChEA3_TFs.csv")){
  ...
  TFs <- map(gene_fns, getTFs, logFC_thresh = 1) %>%
    bind_rows()  %>%
  rename(Query_Name = 'Query Name') %>%
  mutate(specimenName = str_remove(Query_Name, "h.txt"),
         Score = as.numeric(Score))%>%
  dplyr::select(specimenName, TF, Score) %>%
  spread(key = TF, value = Score)
  write_csv(TFs, "RNAseq/Data/ChEA3_TFs.csv")
} else{
...
```{r plotTFsvsPathways, eval = FALSE}
